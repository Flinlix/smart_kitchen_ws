# Smart Kitchen Simulation

**ROS 2 Gazebo simulation environment for the Smart Kitchen robot** — mock and helper nodes that replace real hardware drivers (except for the cameras) so the full logic stack can be developed and tested without physical hardware.

---

## What Is This?

`smart_kitchen_simulation` provides five nodes that bridge the **Gazebo simulation** to the rest of the system. They expose the same ROS2 topics that real hardware would publish, so `smart_kitchen_logic` and all other packages run unchanged in simulation (only cameras are not included in the simulation).

> **Coordinate system:** Gazebo and the robot use different conventions. The smart_kitchen origin sits at Gazebo world offset `(x=0.12, y=1.56, z=2.395)` and the **Z-axis is inverted**. All nodes that translate between frames apply this offset and inversion automatically to reflect the real environment.
...
| Node | Role |
| ---- | ---- |
| `rail_simulator_node` | Replaces the real ELMO drive interface. Subscribes to `/elmo/id1/carriage/position/set` and `/elmo/id1/lift/position/set`, forwards commands to `rail_trajectory_controller` as `JointTrajectory` messages, and publishes position feedback on the corresponding `/get` topics at 10 Hz. The carriage sign is inverted internally to match the controller convention. (the movement of the carriage in simulation is not happening in the same speed as in the real kitchen (future work), but reaches the same positions) |
