...
`smart_kitchen_logic` implements the high-level behavior for a kinova robot arm in a kitchen: it runs **command sequences** (e.g. pick cup → move to sink → drop), drives the **arm** and **gripper**, and can either follow **pre-taught positions** or **dynamically** find cups via ArUco tags and solve Inverse Kinematics to grasp them.


| Mode        | Description                                                                                                          |
| ----------- | -------------------------------------------------------------------------------------------------------------------- |
| **Fixed**   | Uses a sequence of **fixed, taught positions** (waypoints) to grap the cups from the counter or table nearby.     |
| **Dynamic** | **Detects cups** with ArUco tags on them and **solves inverse kinematics** at runtime to capture the cup on the fly. (Only tested and verified to grap cups from a human holding them, not directly grapping cups on the counter) |
...
| **Dynamic** | **Vision module / RGB camera** on the robot gripper, running and publishing image + camera info (e.g. publishing `/camera/color/image_raw` and `/camera/color/camera_info`), using the ArUco tags on cups (node `aruco_all_distance` from aruco_tracker package) |
...
Uses pre-taught positions from `config/waypoints.toml` and a fixed command sequence, that collects cups in a defined order including decision points. Those change the order of collecting the cups or the trajectory the robot is taking to reach the cups, if a human is present. 
Running `command_sequence_client`will directly start the program.
...
Detects cups via ArUco tags, computes IK to reach the tag pose. First runs a detection pose, then collects all the detected cups and brings them to the sink.   
Running the `planning_node` will start the program (and the robot).
...
| Node                                                 | Role                                                                                                                                                                                                                                                                                                                                                                   |
| ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|  `robot_controller` | Exposes the `/move_to_joints` action (defined in `smart_kitchen_interfaces`). Accepts joint-angle goals and forwards them to the joint trajectory controller. Used by both fixed and dynamic stacks. (can handle **emergency stop** by cancelling the current trajectory, which was not fully integrated into the project)                                                                                                                                     |
| `command_executor`                               | **Action server** for `/execute_command` (action defined in `smart_kitchen_interfaces`). Takes high-level command names (e.g. `pick_cup_1`, `drop_cup`), resolves them to waypoint sequences from config, and runs them (joint moves, carriage/lift, gripper). Used by both modes.                                                                                                                                |
...
| `safety_monitor` ⚠️ *(future work)*              | Monitors the horizontal distance between the robot end-effector (via TF2) and the human position from `/human_position`. Publishes `/emergency_stop` (Bool) when the human comes within 0.5 m of the gripper surface. **Not currently used** — human safety is handled at the task level by `human_detection_node` and decision points in `command_sequence_client`. The safety monitor is implemented as a foundation for a proper reactive safety layer in future work. |
